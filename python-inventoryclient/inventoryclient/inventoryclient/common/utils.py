...
def define_command(subparsers, command, callback, cmd_mapper):
    ...
    if _does_command_need_no_wrap(callback):
        ...
        func = _wrapping_formatter_callback_decorator(
            subparser, command, callback)
    else:
        func = callback
        ...
        if wrapping_formatters.WrapperFormatter.is_wrapper_formatter(
            ...
            def sort_key(x):
                return field_formatter.\
                    wrapper_formatter.get_unwrapped_field_value(x)
        else:
            def sort_key(x):
...
def wordwrap_header(field, field_label, formatter):
    """
      Given a field label (the header text for one column) and the word
      wrapping formatter for a column,
    ...
    if not wrapping_formatters.WrapperFormatter.is_wrapper_formatter(
            formatter):
        return field_label
    # go to the column's formatter and ask it what the width should be
    wrapper_formatter = formatter.wrapper_formatter
    actual_width = wrapper_formatter.get_actual_column_char_len(
        wrapper_formatter.get_calculated_desired_width())
...
def pt_builder(field_labels, fields, formatters, paging,
    ...
    class PT_Builder(object):
        ...
        def _row_add(self, obj):
            ...
            if (self.terminal_lines_left - rheight) >= 0 or \
                    self.paged_rows_added == 0:
                self.__add_row_and_obj(row, obj)
                ...
                if s == 'q':
                    self.quit = True
                    return False
                self.terminal_lines_left = \
                    self.terminal_height - self.header_height
                self.build_pretty_table()
                self.__add_row_and_obj(row, obj)
                self.terminal_lines_left -= rheight
        ...
        def get_string(self):
            ...
            if output_width <= self.terminal_width:
                return output
            # At this point pretty Table (self.pt) does not fit the terminal
            # width so let's temporarily turn wrapping off,
            # rebuild the pretty Table with the data unwrapped.
            orig_no_wrap_settings = \
                wrapping_formatters.set_no_wrap_on_formatters(
                    True, self.formatters)
            self.build_pretty_table()
            for o in objs:
                self.add_row(o)
            wrapping_formatters.unset_no_wrap_on_formatters(
                orig_no_wrap_settings)
        ...
        def build_pretty_table(self):
            field_labels = [wordwrap_header(field, field_label, formatter)
                            ...
                            zip(self.fields, self.unwrapped_field_labels, [
                                formatters.get(f, None) for f in self.fields])]
            self.pt = prettytable_builder(
                field_labels, caching=False, print_empty=False)
            self.pt.align = 'l'
            # 2 header border lines + 1 bottom border + 1 prompt
            #     + header data height
...
def print_long_list(objs, fields, field_labels,
                    formatters={}, sortby=0, reversesort=False,
                    no_wrap_fields=[], no_paging=False,
                    printer=default_printer):

    formatters = wrapping_formatters.as_wrapping_formatters(
        objs, fields, field_labels, formatters,
        no_wrap_fields=no_wrap_fields)

    objs = _sort_for_list(objs, fields,
                          formatters=formatters,
                          sortby=sortby,
                          reversesort=reversesort)

    pt = pt_builder(field_labels, fields, formatters, not no_paging,
                    printer=printer)
...
def print_list(objs, fields, field_labels, formatters={}, sortby=0,
    ...
    return print_long_list(
        objs, fields, field_labels, formatters=formatters, sortby=sortby,

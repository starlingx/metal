...
The basic idea is:
   ...
   Once celldata is retrieved and ready to display, first iterate through
   the celldata and word wrap it so that fits programmer desired column widths.
   The WrapperFormatter objects fill this role.

   Once the celldata is formatted to their desired widths,
   then it can be passed to the existing prettyTable code base for rendering.
...
wordsep_re = re.compile(
    ...
    r'\.|'
    r':|'
    r'[^\s\w]*\w+[^0-9\W]-(?=\w+[^0-9\W])|'  # hyphenated word
    r'(?<=[\w\!\"\'\&\.\,\?])-{2,}(?=\w))')  # em-dash
...
def is_uuid_field(field_name):
    ...
    """
    if field_name is not None and field_name in ["uuid", "UUID"] or \
            field_name.endswith("uuid"):
        return True
    return False
...
class WrapperContext(object):
    ...
    def is_table_too_wide(self):
        """
        :return: True if calculated table width is too wide
...
def field_value_function_factory(formatter, field):
    ...
    def field_value_function_builder(data):
        if isinstance(data, dict):
            formatter.get_field_value = \
                lambda celldata: celldata.get(field, None)
        else:
            formatter.get_field_value = \
                lambda celldata: getattr(celldata, field)
...
class WrapperFormatter(object):
    ...
    def get_remaining_row_chars(self):
        ...
        remaining_chrs_in_row = \
            (self.ctx.get_terminal_width() -
    ...
    def get_actual_column_char_len(self, desired_char_len,
...
class WrapperFixedWidthFormatter(WrapperLambdaFormatter):
    ...
    def __init__(self, ctx, field, width):
        super(WrapperFixedWidthFormatter, self).__init__(
            ctx, field,
            lambda data: self.text_wrap(
                str(data), self.get_calculated_desired_width()))
        self.width = width
...
class WrapperPercentWidthFormatter(WrapperFormatter):
    ...
    def get_basic_desired_width(self):
        width = int((self.ctx.get_terminal_width() -
                     self.ctx.non_data_chrs_used_by_table) *
...
class WrapperWithCustomFormatter(WrapperLambdaFormatter):
    ...
    def __init__(self, ctx, field, custom_formatter, wrapper_formatter):
        super(WrapperWithCustomFormatter, self).__init__(
            ctx, None,
...
def wrapper_formatter_factory(ctx, field, formatter):
    ...
    formatter = type = dict :      Builds a WrapperWithCustomFormatter that
                                   ...
                                   and then apply a wrapping formatter
                                   to the result

                                    ex: this dict
                                        {"formatter" : captializeFunction,,
                                         "wrapperFormatter":  .12}
                                    will apply the captializeFunction
                                    to the column celldata and then
                                    wordwrap at 12 % of terminal width

    :param ctx:  the WrapperContext that the built WrapperFormatter will use
    :param field:   name of field (column_ that the WrapperFormatter
                    will execute on
    :param formatter: specifies type and input for WrapperFormatter
                      that will be built
    ...
    if isinstance(formatter, dict):
        if "wrapperFormatter" in formatter:
            embedded_wrapper_formatter = wrapper_formatter_factory(
                ctx, None, formatter["wrapperFormatter"])
        elif "hard_width" in formatter:
            embedded_wrapper_formatter = WrapperFixedWidthFormatter(
                ctx, field, formatter["hard_width"])
            embedded_wrapper_formatter.min_width = formatter["hard_width"]
        else:
            embedded_wrapper_formatter = WrapperFormatter(
                ctx, None)  # effectively a NOOP width formatter
        if "formatter" not in formatter:
            return embedded_wrapper_formatter
        custom_formatter = formatter["formatter"]
        wrapper = WrapperWithCustomFormatter(
            ctx, field, custom_formatter, embedded_wrapper_formatter)
        return wrapper

    raise Exception("Formatter Error! Unrecognized formatter {} "
                    "for field {}".format(formatter, field))


def build_column_stats_for_best_guess_formatting(objs, fields, field_labels,
                                                 custom_formatters={}):
    class ColumnStats(object):
        def __init__(self, field, field_label, custom_formatter=None):
            ...
            if custom_formatter:
                ...
                self.get_field_value = \
                    field_value_function_factory(self, field)
        ...
        def add_value(self, value):
            ...
            if self.count > 0:
                self.average_width = float(
                    self.total_width) / float(self.count)

        def set_max_percent(self, max_total_width):
            if max_total_width > 0:
                self.max_percent = float(
                    self.max_width) / float(max_total_width)

        def set_avg_percent(self, avg_total_width):
            if avg_total_width > 0:
                self.average_percent = float(
                    self.average_width) / float(avg_total_width)
    ...
    for i in range(0, len(fields)):
        stats[fields[i]] = ColumnStats(
            fields[i], field_labels[i], custom_formatters.get(fields[i]))
...
def build_best_guess_formatters_using_average_widths(
        objs, fields, field_labels,
        custom_formatters={}, no_wrap_fields=[]):

    column_info = build_column_stats_for_best_guess_formatting(
        objs, fields, field_labels, custom_formatters)
    format_spec = {}
    ...
    for f in [ff for ff in fields if ff not in no_wrap_fields]:
        format_spec[f] = float(
            column_info["stats"][f].average_width) / total_avg_width
        custom_formatter = custom_formatters.get(f, None)
        if custom_formatter:
            format_spec[f] = {"formatter": custom_formatter,
                              "wrapperFormatter": format_spec[f]}
...
def build_best_guess_formatters_using_max_widths(objs, fields, field_labels,
    ...
    for f in [ff for ff in fields if ff not in no_wrap_fields]:
        format_spec[f] = \
            float(column_info["stats"][f].max_width) / float(column_info["total_max_width"])  # noqa
        custom_formatter = custom_formatters.get(f, None)
        if custom_formatter:
            format_spec[f] = {"formatter": custom_formatter,
                              "wrapperFormatter": format_spec[f]}
...
def as_wrapping_formatters(objs, fields, field_labels, formatters,
                           no_wrap=None, no_wrap_fields=[]):
    """This function is the entry point for building the "best guess"
       ...
       columns widths should be for the table celldata.  It does this by
       collecting various stats on the celldata (min, max average width of
       column celldata) and from this celldata decides the desired widths
       and the minimum widths.

       Given a list of formatters and the list of objects (objs),  this
       function first determines if we need to augment the passed formatters
       with word wrapping formatters.
       If the no_wrap parameter or global no_wrap flag is set,
       then we do not build wrapping formatters.  If any of the formatters
       within formatters is a word wrapping formatter,
       then it is assumed no more wrapping is required.
    ...
    :return: When no wrapping is required, the formatters parameter is returned
              ...
              When wrapping is required, best-guess word wrapping formatters
              are returned with original parameter formatters embedded in the
              word wrapping formatters
    """
    no_wrap = is_nowrap_set(no_wrap)
    ...
    format_spec = build_best_guess_formatters_using_average_widths(
        objs, fields, field_labels, formatters, no_wrap_fields)

    formatters = build_wrapping_formatters(
        objs, fields, field_labels, format_spec)

    return formatters


def build_wrapping_formatters(objs, fields, field_labels, format_spec,
                              add_blank_line=True,
                              no_wrap=None,
                              use_max=False):
    """
      A convenience function for building all wrapper formatters that
      will be used to format a CLI's output when its rendered
      in a prettyTable object.

      It iterates through the keys of format_spec and
      calls wrapperFormatterFactory to build
      ...
      Its best to show by example parameters:
        ...
        format_spec =  {
                            "uuid"                       : .10,
                                # float = so display as 10% of terminal width
                            "timestamp"                  : .08,
                            "state"                      : .08,
                            "event_log_id"               : .07,
                            "reason_text"                : .42,
    ...
    :param fields: fields (attributes of the celldata) that will be
                   displayed in the table
    :param field_labels: column (field headers)
    :param format_spec: dict specify formatter for each column (field)
    :param add_blank_line: default True, when tru adds blank line to column
                           if it wraps, aids readability
    :param no_wrap: default False, when True turns wrapping off but does
                    not suppress other custom formatters
    :param use_max
    ...
    if len(fields) != len(field_labels):
        raise Exception("Error in buildWrappingFormatters: "
                        ...
                        " they must be the same length!".format(
                            len(fields), len(field_labels)))
    field_to_label = {}
    ...
    if not format_spec:
        if use_max:
            format_spec = build_best_guess_formatters_using_max_widths(
                objs, fields, field_labels)
        else:
            format_spec = build_best_guess_formatters_using_average_widths(
                objs, fields, field_labels)
    ...
    for k in format_spec.keys():
        ...
        wrapper_formatter = wrapper_formatter_factory(
            ctx, k, format_spec_for_k)
        if wrapper_formatter.min_width <= 0:
            ...
            if is_uuid_field(k):  # special case
                ...
                column_data = [
                    str(wrapper_formatter.get_unwrapped_field_value(data))
                    for data in objs]
                widest_word_in_column = max(
                    [get_biggest_word(d) + " "
...
def set_no_wrap_on_formatters(no_wrap, formatters):
    ...
    for k, f in formatters.iteritems():
        if WrapperFormatter.is_wrapper_formatter(f):
            formatter_no_wrap_settings[k] = (
                f.wrapper_formatter.no_wrap, f.wrapper_formatter)
...
def unset_no_wrap_on_formatters(orig_no_wrap_settings):
    ...
    formatter_no_wrap_settings = \
        orig_no_wrap_settings["formatter_no_wrap_settings"]
...
def _simpleTestHarness(no_wrap):
    ...
    utils.print_list(
        logs, fields, field_labels, formatters=formatters, sortby=6,

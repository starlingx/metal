...
CREATION_ATTRIBUTES = ['host_uuid', 'node_uuid', 'cpu', 'core', 'thread',
...
class Cpu(base.Resource):
    def __repr__(self):
        return "<cpu %s>" % self._info


class CpuManager(base.Manager):
    resource_class = Cpu

    def list(self, host_id):
        path = '/v1/hosts/%s/cpus' % host_id
        return self._list(path, "cpus")

    def get(self, cpu_id):
        path = '/v1/cpus/%s' % cpu_id
        try:
...
def check_core_functions(personality, cpus):
    platform_cores = 0
    vswitch_cores = 0
    vm_cores = 0
    for cpu in cpus:
    ...
    if host.cpus:
        ...
        for cpu in host.cpus:
            ...
            if cpu.numa_node not in host.node_min_max_cores:
                host.node_min_max_cores[cpu.numa_node] = \
                    {'min': 99999, 'max': 0}
            ...
            if cpu.numa_node not in core_assignment[cpu.allocated_function]:
                ...
                core_assignment[
                    cpu.allocated_function][cpu.numa_node].append(int(cpu.cpu))
                number_of_cores[cpu.allocated_function][cpu.numa_node] = \
                    number_of_cores[cpu.allocated_function][cpu.numa_node] + 1

...
class LLDPNeighbour(base.InventoryObject,
    ...
    def get_by_uuid(cls, context, uuid):
        db_lldp_neighbour = cls.dbapi.lldp_neighbour_get(uuid)
        return cls._from_db_object(context, cls(), db_lldp_neighbour)
    ...
    def list(cls, context, limit=None, marker=None, sort_key=None,
        ...
        """Return a list of LLDPNeighbour objects.

        :param cls: the :class:`LLDPNeighbour`
        ...
        :returns: a list of :class:`LLDPNeighbour` object.

        """
        db_lldp_neighbours = cls.dbapi.lldp_neighbour_get_list(
            filters=filters,
            limit=limit, marker=marker, sort_key=sort_key, sort_dir=sort_dir)
        return cls._from_db_object_list(context, db_lldp_neighbours)

    @classmethod
    def get_by_host(cls, context, host_uuid,
                    limit=None, marker=None,
                    sort_key=None, sort_dir=None):
        db_lldp_neighbours = cls.dbapi.lldp_neighbour_get_by_host(
            host_uuid,
            limit=limit,
            marker=marker,
            sort_key=sort_key,
    ...
    def get_by_port(cls, context, port_uuid,
                    limit=None, marker=None,
                    sort_key=None, sort_dir=None):
        db_lldp_neighbours = cls.dbapi.lldp_neighbour_get_by_port(
            port_uuid,
            limit=limit,
            marker=marker,
            sort_key=sort_key,
    ...
    def create(self, context, portid, hostid, values):
        ...
        db_lldp_neighbour = self.dbapi.lldp_neighbour_create(
            portid, hostid, values)
        return self._from_db_object(self._context, self, db_lldp_neighbour)
